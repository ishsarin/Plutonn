import React from 'react'
import { LuLogIn } from "react-icons/lu";
export const LeftSide = () => {
  return (
    <>
    <div className="container left-side">
    <div>
      <div>
      
      <span>  <img src="https://www.plutonn.com/static/media/Mask.c87a2edc5f7bc40a251cbff35040b1cb.svg" alt="" className='smile-img'/></span>
      
        <img src="data:image/png;base64," alt="" height={60} width={60}/>
      </div>
    </div>
    <div className='login-text-wrapper'>
    <div className='login-text'><h2>You Haven't Logged In</h2>
      <h3>take your username now</h3></div>
      <div>Click on the login button and make yourself a part of this wonderful community.</div>
    </div>
    <div style={{position:"relative",bottom:"20px"}}><button className="btn login-button"><LuLogIn size={25} color='white'/> Login</button></div>
    </div>
      
</>
      
  )
}
